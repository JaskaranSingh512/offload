...
export const Chip = ({
  ...
  <span className={`chip ${tone === "teal" ? "chip-teal" : ""}`}>
    {dot && <span className="chip-dot" style={dot === true ? {} : { background: dot as string }} />}
    {children}
  </span>
);

export const ChannelIcon = ({ channel, size = 14 }: { channel: ChannelId; size?: number }) => {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} width="100%" style={{ display: "block" }}>
      ...
      {data.map(
        (d, i) =>
          i % 2 === 0 && (
            <text key={`l${i}`} x={pad.left + i * xStep} y={h - 8} textAnchor="middle" fontSize="10" fill="var(--text-faint)" fontFamily="var(--font-mono)">
              {d.label}
            </text>
          ),
      )}
