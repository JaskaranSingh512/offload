...
export const ContentCalendar = () => {
  ...
  const postsByDay = filtered.reduce<Record<number, Post[]>>((acc, p) => {
        ...
        return (
          <div key={dayNum} className={`cal-day ${isToday ? "today" : ""}`}>
            <div className="cal-day-head">
              ...
              <span className="cal-day-dow">{isToday ? <span style={{ color: "var(--teal-deep)" }}>TODAY</span> : date.dow}</span>
            </div>
            {dayPosts
              ...
              .map((p) => {
                ...
                return (
                  <div key={p.id} className="cal-post" style={{ borderLeftColor: meta.color }} onClick={() => openPost(p)}>
    ...
    <div className="main-inner">
      ...
      <div className="cal-head">
        <div className="cal-legend">
          ...
          {(Object.entries(channelMeta) as [ChannelId, (typeof channelMeta)[ChannelId]][]).map(([id, m]) => (
            <button
              key={id}
      ...
      <div className="card" style={{ marginTop: 24, padding: 20, display: "flex", gap: 16, alignItems: "center", background: "var(--cream)" }}>
        ...
        <p style={{ margin: 0, fontSize: 13.5, color: "var(--espresso)", flex: 1 }}>
          <strong>Tip:</strong> Click any post to preview the full content, edit copy, swap visuals, or reschedule. Anything you change will be reflected on the live calendar instantly.
        </p>
      </div>
    </div>
  );
