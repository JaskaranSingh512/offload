...
The build runbook is `EXECUTION_PLAN.md`, ordered as **Phase 0 → 8**. **Active scope =
the 12-Hour MVP folded into that plan — read `EXECUTION_PLAN.md` §0.5 FIRST** (Next.js
rails, all 4 channels, onboarding brand-doc upload → AI channel recommendation, `.md`/`.txt`
only; Canva render + GitHub auth are deferred decisions). §0.5 overrides any phase detail it
conflicts with. Execute phases **in order** on the build branch. A phase is **done only when
its Verification gate in EXECUTION_PLAN.md passes** — run that exact gate command and confirm
the pass condition before moving on. Do not start phase N+1 until phase N's gate is green.
...
| 3 | Schema + types + seed | all 13 tables exist (incl. `brands` doc/recommendation columns, §0.5); every table reads ≥1 row, `posts` ≥20; types regenerate; `typecheck` exits 0 (§4) |
| 4 | Surfaces | mock renders all 5 surfaces + drawer + chat with no console errors; onboarding brand-doc upload + AI channel suggestion present; live reads work with `USE_MOCK=false`; `./verify.sh` exits 0 (§5) |
| 5 | AI Route Handlers | `/api/generate` streams + golden fallback inserts 8 posts; `/api/chat-edit` returns a valid `PostContent` patch; `/api/analyze` returns `{industry, recommended_channels[], channel_rationale}`; `typecheck` 0 (§6) |
...
- 4 channels only: Reddit, TikTok, Instagram, X. No others in v1. Onboarding uploads a brand
  doc (`.md`/`.txt`) and the AI **recommends** which of the 4 to lead with — it pre-selects,
  never restricts (see EXECUTION_PLAN §0.5).
...
- Canva slideshow render and real GitHub auth are **deferred decisions** — stop and ask before
  building them (EXECUTION_PLAN §0.5).
