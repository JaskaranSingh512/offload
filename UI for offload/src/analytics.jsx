const Analytics = ({ setRoute }) => {
  const { channelStats, formatStats, working, recommendations, channelMeta } = window.brewLab;
  ...
  const signupData = dailyData.map((d) => ({ ...d, value: Math.round(d.value / 60) }));
  ...
  return (
    <div className="main-inner">
      <PageHead
        ...
        actions={
          <>
            <button className="btn btn-secondary"><I.Copy size={13}/> Export PDF</button>
            <button className="btn btn-primary" onClick={() => setRoute('builder')}><I.Sparkle size={13}/> Brief next campaign</button>
          </>
        }
      />
      ...
      <div className="recap-hero">
        <div>
          ...
          <h2>This campaign was a <span className="em">win.</span></h2>
          <p style={{ fontSize: 16, color: 'var(--text-muted)', lineHeight: 1.55, margin: 0, maxWidth: 460 }}>
            815k impressions and 1,235 signups across 35 posts — meaningfully ahead of forecast on three of your four channels. Most of the lift came from two things we should keep doing.
          </p>
          <div className="flex gap-2 mt-6">
            <Chip tone="teal" dot>+34% vs forecast</Chip>
            <Chip>$0.42 CAC</Chip>
        ...
        <div className="rh-stats">
          <div className="rh-stat">
            ...
            <span className="delta"><I.ArrowUp size={11}/> +34%</span>
          </div>
          <div className="rh-stat">
            <p className="label">Signups</p>
            <p className="value">1,235</p>
            <span className="delta"><I.ArrowUp size={11}/> +61%</span>
          </div>
          <div className="rh-stat">
            <p className="label">Engagement</p>
            <p className="value">6.8%</p>
            <span className="delta"><I.ArrowUp size={11}/> +1.4pt</span>
          </div>
          <div className="rh-stat">
            <p className="label">Cost per signup</p>
            <p className="value">$0.42</p>
            <span className="delta"><I.ArrowDown size={11}/> –52%</span>
          </div>
        </div>
      </div>
        ...
        <div className="card card-pad-lg">
          <div className="card-head">
            ...
            <div className="flex gap-3 items-center" style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              <span className="flex items-center gap-2"><span style={{ width: 12, height: 2, background: 'var(--teal)', display: 'inline-block' }}/> Impressions</span>
              <span className="flex items-center gap-2"><span style={{ width: 12, height: 2, background: 'var(--espresso-50)', borderTop: '1px dashed', display: 'inline-block' }}/> Forecast</span>
            </div>
          </div>
          <LineChart
            data={dailyData}
          ...
          <div className="flex gap-6 mt-6" style={{ fontSize: 12, color: 'var(--text-muted)', borderTop: '1px solid var(--border)', paddingTop: 14 }}>
            <div>
              <div className="eyebrow" style={{ marginBottom: 4 }}>Peak day</div>
              <div style={{ fontSize: 14, color: 'var(--espresso)', fontWeight: 500 }}>May 18 · 80,400 imp</div>
            </div>
            <div>
              <div className="eyebrow" style={{ marginBottom: 4 }}>Best post</div>
              <div style={{ fontSize: 14, color: 'var(--espresso)', fontWeight: 500 }}>Reddit AMA · 142k</div>
            </div>
            <div>
              <div className="eyebrow" style={{ marginBottom: 4 }}>Avg daily growth</div>
            ...
            {channelStats.map(c => {
              ...
              return (
                <div key={c.channel} className="channel-bar" style={{ padding: '8px 0' }}>
                  <div className="cb-icon" style={{ background: meta.color, width: 22, height: 22, borderRadius: 6 }}><meta.Icon size={11}/></div>
                  <span className="cb-name" style={{ width: 76, fontSize: 12.5 }}>{meta.name}</span>
                  <span className="text-mono" style={{ fontSize: 12, color: 'var(--text-muted)', flex: 1 }}>{c.signups} signups · {c.ctr}% CTR</span>
      ...
      <div className="analytics-grid">
        <div className="card card-pad-lg">
          <div className="card-head">
            ...
            <span className="chip"><I.Sparkle size={10}/> Stars are top performers</span>
          </div>
              ...
              {formatStats.map((f, i) => {
                ...
                return (
                  <tr key={i} className={f.winner ? 'winner-row' : ''}>
                    ...
                    <td style={{ textAlign: 'right', fontWeight: 500, fontFamily: 'var(--font-serif)', fontSize: 16, color: 'var(--espresso)' }}>{f.signups}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className="card card-pad-lg">
          <div className="card-head">
            ...
            <span className="chip chip-teal" dot>4 themes</span>
          </div>
        ...
        <div className="rc-head">
          ...
          <div>
            ...
            <div style={{
              ...
              marginTop: 2,
            }}>Your next move, ranked.</div>
          </div>
          <button className="btn btn-on-dark" style={{
            marginLeft: 'auto',
            background: 'var(--teal)',
            color: 'white',
