const Scripts = ({ setRoute }) => {
  const { founderScripts } = window.brewLab;
  ...
  return (
    <div className="main-inner">
      <PageHead
        ...
        actions={
          <>
            <button className="btn btn-secondary"><I.Plus size={13}/> Request new script</button>
            <button className="btn btn-primary"><I.Film size={13}/> Open in shooter mode</button>
          </>
        }
      />
      ...
      <div className="flex gap-2 items-center" style={{ marginBottom: 24 }}>
        ...
        <div style={{ marginLeft: 'auto' }} className="text-mono text-muted" style={{ fontSize: 12 }}>
        ...
        {founderScripts.map(s => (
          <article className="script-card" key={s.id}>
            <div className="script-card-head">
              <div style={{ flex: 1 }}>
                <div className="eyebrow" style={{ marginBottom: 6 }}>{s.angle}</div>
                <h3 className="sc-title">{s.title}</h3>
                <div className="sc-meta">
                  <span><I.Clock size={11} style={{ verticalAlign: '-1px' }}/> {s.duration}</span>
                  <span>·</span>
                  <span>{s.platforms.join(', ')}</span>
                </div>
              </div>
              <button className="btn btn-ghost btn-sm" title="Copy"><I.Copy size={13}/></button>
            </div>

            <p className="sc-hook">"{s.hook}"</p>

            <div>
              <div className="eyebrow" style={{ marginBottom: 8 }}>Beats</div>
              <div className="sc-beats">
                {s.beats.map((b, i) => (
                  <div key={i} className="sc-beat">
                    <span className="beat-time">{b.time}</span>
                    <span className="beat-text" dangerouslySetInnerHTML={{
                      __html: b.text.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
                    }} />
                  </div>
                ))}
              </div>
            ...
            <div style={{
              ...
              <strong style={{ color: 'var(--espresso)', fontStyle: 'normal' }}>Shot note · </strong>{s.note}
            </div>

            <div className="sc-foot">
              <button className="btn btn-secondary btn-sm"><I.Edit size={12}/> Edit</button>
              <button className="btn btn-secondary btn-sm"><I.Wand size={12}/> Variations</button>
      ...
      <div className="card" style={{ marginTop: 24, padding: 28, textAlign: 'center', background: 'var(--cream)' }}>
        ...
        <button className="btn btn-primary"><I.Sparkle size={13}/> Brief a new script</button>
      </div>
    </div>
  );
};
