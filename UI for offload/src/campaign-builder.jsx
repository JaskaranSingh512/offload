const CampaignBuilder = ({ setRoute }) => {
  const [goal, setGoal] = useState('awareness');
  const [duration, setDuration] = useState('2w');
  const [frequency, setFrequency] = useState('balanced');
  const [channels, setChannels] = useState({ reddit: true, tiktok: true, instagram: true, x: false });
      ...
      <div className="builder-grid">
        <div className="flex flex-col gap-4">
          ...
          <div className="card">
            <div className="card-head">
              ...
              <span className="text-muted" style={{ fontSize: 12 }}>What success looks like</span>
            </div>
          ...
          <div className="card">
            ...
            <div className="seg">
              <button className={duration === '1w' ? 'on' : ''} onClick={() => setDuration('1w')}>1 week</button>
              <button className={duration === '2w' ? 'on' : ''} onClick={() => setDuration('2w')}>2 weeks</button>
              <button className={duration === '4w' ? 'on' : ''} onClick={() => setDuration('4w')}>4 weeks</button>
              <button className={duration === 'custom' ? 'on' : ''} onClick={() => setDuration('custom')}>Custom…</button>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="seg">
              <button className={frequency === 'light' ? 'on' : ''} onClick={() => setFrequency('light')}>Light · 1/day</button>
              <button className={frequency === 'balanced' ? 'on' : ''} onClick={() => setFrequency('balanced')}>Balanced · 2-3/day</button>
              <button className={frequency === 'aggressive' ? 'on' : ''} onClick={() => setFrequency('aggressive')}>Aggressive · 4+/day</button>
            </div>
        ...
        <div className="card summary-card">
          <div className="card-head">
            ...
            <span className="chip chip-teal"><span className="chip-dot"/> Ready</span>
          </div>
          ...
          <div className="text-muted" style={{ fontSize: 13, marginBottom: 18 }}>for <strong style={{ color: 'var(--espresso)' }}>Brew Lab</strong></div>
          ...
          <div className="summary-row">
            ...
            <span className="sr-value flex gap-2 items-center">
              {Object.keys(channels).filter(c => channels[c]).map(c => {
                const meta = window.brewLab.channelMeta[c];
                return (
          ...
          <div className="summary-row">
            ...
            <span className="sr-value" style={{ fontFamily: 'var(--font-serif)', fontSize: 24, color: 'var(--teal-deep)' }}>{estPosts}</span>
          </div>
          <div className="summary-row">
