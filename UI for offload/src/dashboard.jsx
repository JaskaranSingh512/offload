const Dashboard = ({ setRoute, openPost }) => {
  const { posts, TODAY_DAY, channelMeta, dateLabels } = window.brewLab;

  // "Next up" posts (today + tomorrow, upcoming only)
  const nextUp = posts
    .filter(p => p.day === TODAY_DAY || p.day === TODAY_DAY + 1)
    .slice(0, 5);

  // Channel breakdown (count of posts per channel)
  const channelCounts = posts.reduce((acc, p) => {
    ...
    <div className="main-inner">
      ...
      <div className="kpi-row">
        <div className="kpi">
          ...
          <span className="kpi-delta"><I.ArrowUp size={11}/> +24% vs prior</span>
        </div>
        <div className="kpi">
          <p className="kpi-label">Signups (7d)</p>
          <p className="kpi-value">312</p>
          <span className="kpi-delta"><I.ArrowUp size={11}/> +41%</span>
        </div>
        <div className="kpi">
          <p className="kpi-label">Engagement rate</p>
          <p className="kpi-value">6.4%</p>
          <span className="kpi-delta"><I.ArrowUp size={11}/> +1.2pt</span>
        </div>
        <div className="kpi">
          <p className="kpi-label">Posts scheduled</p>
          <p className="kpi-value">35</p>
          <span className="kpi-delta" style={{ background: 'var(--cream)', color: 'var(--espresso)' }}>over 14 days</span>
        </div>
      </div>
      ...
      <div className="dash-grid">
          ...
          <div className="ac-inner">
            ...
            <div className="ac-stats">
              <div className="ac-stat">
                ...
                <p className="value">8 <span style={{ fontSize: 14, color: 'rgba(245,239,230,0.5)' }}>/ 35</span></p>
              </div>
              <div className="ac-stat">
                <p className="label">Next post</p>
                <p className="value" style={{ fontSize: 22 }}>9:00 AM</p>
              </div>
              <div className="ac-stat">
                <p className="label">Forecast</p>
                <p className="value" style={{ fontSize: 22 }}>~58k <span style={{ fontSize: 13, color: 'rgba(245,239,230,0.5)' }}>imp / day</span></p>
              </div>
            </div>
            ...
            <div className="ac-actions">
              <button className="btn btn-on-dark" onClick={() => setRoute('calendar')}>Open campaign →</button>
              <button className="btn btn-on-dark ghost" onClick={() => setRoute('analytics')}>View live analytics</button>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          <div className="card-head">
            ...
            <button className="card-link" onClick={() => setRoute('calendar')}>See all <I.ChevronRight size={12}/></button>
          </div>
          {nextUp.map(p => (
            <div key={p.id} className="next-up-row" style={{ cursor: 'pointer' }} onClick={() => openPost(p)}>
              ...
              <div className="nu-meta">
                ...
                <p className="nu-sub">{channelMeta[p.channel].name} · {p.type}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="dash-grid" style={{ marginTop: 20 }}>
        <div className="card">
          ...
          <LineChart
            data={trendData.map(d => ({ ...d, value: d.value * 7.6 }))}
            height={200}
          />
        </div>
        ...
        <div className="card">
          <div className="card-head">
            ...
            <button className="card-link" onClick={() => setRoute('analytics')}>Deep dive <I.ChevronRight size={12}/></button>
          </div>
            ...
            .map(([ch, count]) => {
              ...
              return (
                <div key={ch} className="channel-bar">
                  <div className="cb-icon" style={{ background: meta.color }}><meta.Icon size={14} /></div>
      ...
      <div className="card" style={{ marginTop: 20, padding: 24, display: 'flex', gap: 24, alignItems: 'center' }}>
        ...
        <div style={{ flex: 1 }}>
          <div className="eyebrow" style={{ color: 'var(--teal-deep)' }}>Tether suggests</div>
          <p style={{ fontFamily: 'var(--font-serif)', fontSize: 22, margin: '4px 0 4px', color: 'var(--espresso)', letterSpacing: '-0.01em', lineHeight: 1.2 }}>
        ...
        <div className="flex gap-2">
          ...
          <button className="btn btn-teal">Draft follow-up <I.Arrow size={12}/></button>
        </div>
      </div>
    </div>
  );
